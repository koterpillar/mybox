...
data TrackerSession :: Effect where
  ...
  TrkAdd :: PackageName p => p -> Path -> TrackerSession m ()
...
trkAdd :: (PackageName p, TrackerSession :> es) => p -> Path -> Eff es ()
...
tfMake :: PackageName p => p -> Path -> TrackedFile
...
drvTracker :: Driver :> es => Path -> Eff (Tracker : es) a -> Eff es a
