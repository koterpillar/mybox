...
masList :: forall s es. (App es, IsSystemPackage s) => Eff es (Map Text PackageVersion)
masList = parseList <$> runMas @s drvRunOutput ["list"]

masOutdated :: forall s es. (App es, IsSystemPackage s) => Eff es (Map Text PackageVersion)
masOutdated = parseList <$> runMas @s drvRunOutput ["outdated"]
...
-}
parseInfo :: Text -> Text
parseInfo info = case Text.words info of
...
masPackageInfo Nothing = Map.union <$> masOutdated @s <*> masList @s
masPackageInfo (Just pkg) = do
  local <- masList @s
  let localVersion = Map.lookup pkg local >>= (.installed)
  version <- parseInfo <$> runMas @s drvRunOutput ["info", pkg]
  pure $
    Map.singleton pkg $
      PackageVersion
