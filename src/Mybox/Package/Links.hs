...
lpRemoteVersion :: Driver :> es => LinksPackage -> Eff es RemoteRelease
lpRemoteVersion p = (\version -> RemoteRelease{version, timestamp = Nothing}) . Text.intercalate "#" . map (.text) . Set.toList <$> paths p -- FIXME: hash
